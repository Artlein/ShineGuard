...
<div id="authGateModal" class="modal" style="display:none; position:fixed; inset:0; background:rgba(15,23,42,0.6); backdrop-filter:blur(12px) saturate(160%); z-index:10000; align-items:center; justify-content:center;">
    <div class="glass-panel modal-spring" style="max-width: 440px; width: 90%; background: var(--sg-glass); padding: 2.5rem; text-align: center;">
        ...
        <div class="input-group" style="text-align: left; margin-bottom: 1.5rem; position: relative;">
            <label style="font-size: 10px; letter-spacing: 0.1em; color: var(--sg-blue);">CREDENTIAL VERIFICATION</label>
            <input type="password" id="gatePassword" placeholder="Admin Password" style="width:100%; background:rgba(255,255,255,0.05); border:1px solid var(--sg-glass-border); color:var(--sg-text); height:48px; padding:0 1.25rem; border-radius:14px; font-weight:600; outline:none; padding-right: 50px;">
            <button type="button" onclick="toggleGatePassword()" style="position: absolute; right: 10px; top: 32px; background: none; border: none; color: var(--sg-text-dim); cursor: pointer; display: flex; align-items: center; justify-content: center; padding: 4px; transition: color 0.2s;" onmouseenter="this.style.color='var(--sg-text)'" onmouseleave="if(document.getElementById('gatePassword').type==='password') this.style.color='var(--sg-text-dim)'">
